...
export default function CreateProject() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!token) { Alert.alert("Error", "Login session missing. Please login again."); return; }
  ...
  const handleCreateProject = async () => {
    if (!projectName.trim()) { Alert.alert("Error", "Enter project name"); return; }
    if (!selectedUser) { Alert.alert("Error", "Select user"); return; }

    let formattedDueDate = null;
    if (dueDate.trim()) {
      const parsedDate = new Date(dueDate);
      if (isNaN(parsedDate.getTime())) {
        Alert.alert("Error", "Enter due date in YYYY-MM-DD format"); return;
      }
    ...
    try {
      ...
      if (!token || !adminId) { Alert.alert("Error", "Login session missing. Please login again."); return; }
      ...
      Alert.alert("Success ✅", "Project created successfully!", [
        { text: "OK", onPress: () => {
          setProjectName(""); setUserSearch(""); setDueDate("");
          setSelectedUser(null); setShowUserDropdown(false);
        }}
      ]);
    } catch (error: any) {
  ...
  const isReady = projectName.trim() && selectedUser;
      ...
      >
        ...
        <View style={styles.card}>
          <Text style={styles.label}>
            <Ionicons name="folder-outline" size={14} color="#CBD5F5" /> Project Name *
          </Text>
          <TextInput
        ...
        <View style={styles.card}>
          <Text style={styles.label}>
            <Ionicons name="person-outline" size={14} color="#CBD5F5" /> Select User *
          </Text>

          {selectedUser && (
            <View style={styles.selectedBadge}>
              <Ionicons name="checkmark-circle" size={16} color="#00A693" />
              <Text style={styles.selectedBadgeText}>{selectedUserName}</Text>
          ...
          {showUserDropdown && filteredUsers.length > 0 && (
            <View style={styles.dropdown}>
              {filteredUsers.map((item) => (
                <TouchableOpacity
                  key={item._id}
                  style={[styles.dropdownItem, selectedUser === item._id && styles.dropdownItemSelected]}
                  onPress={() => {
                ...
                >
                  <View style={styles.dropdownItemLeft}>
                    <View style={styles.dropdownAvatar}>
                      <Text style={styles.dropdownAvatarText}>{item.name.charAt(0).toUpperCase()}</Text>
                    </View>
                    <View>
                      <Text style={styles.dropdownItemText}>{item.name}</Text>
                      {item.department && <Text style={styles.dropdownItemSub}>{item.department}</Text>}
                    </View>
                  </View>
                  {selectedUser === item._id && <Ionicons name="checkmark" size={16} color="#00A693" />}
                </TouchableOpacity>
              ))}
            </View>
          )}
        ...
        <View style={styles.card}>
          <Text style={styles.label}>
            <Ionicons name="calendar-outline" size={14} color="#CBD5F5" /> Project Due Date (optional)
          </Text>
          <TextInput
          ...
          <View style={styles.summaryCard}>
            ...
            {selectedUserName ? (
              <View style={styles.summaryRow}>
                ...
                <Text style={styles.summaryText}>{selectedUserName}</Text>
              </View>
            ) : null}
            {dueDate ? (
              <View style={styles.summaryRow}>
                <Ionicons name="calendar" size={14} color="#9CA3AF" />
                <Text style={styles.summaryText}>{dueDate}</Text>
              </View>
            ) : null}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  label: { color: "#CBD5F5", fontSize: 13, fontWeight: "600", marginBottom: 10 },
  input: {
  ...
  selectedBadgeText: { color: "#00A693", fontSize: 13, fontWeight: "600", flex: 1 },
  dropdown: {
    marginTop: 6, backgroundColor: "#081B43",
    borderRadius: 10, overflow: "hidden", maxHeight: 200,
  },
  dropdownItem: {
    flexDirection: "row", alignItems: "center", justifyContent: "space-between",
    padding: 12, borderBottomWidth: 0.5, borderBottomColor: "#1E3A8A",
  },
  ...
  summaryCard: {
    backgroundColor: "#0F2A5F", borderRadius: 16, padding: 14,
    marginBottom: 14, borderLeftWidth: 3, borderLeftColor: "#00A693",
  },
  summaryTitle: {
    color: "#CBD5F5", fontSize: 13, fontWeight: "700",
