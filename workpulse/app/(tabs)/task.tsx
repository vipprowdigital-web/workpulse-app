...
function AdminTaskScreen() {
  const [activeSection, setActiveSection] = useState<
    "assignedHistory" | "myTask" | "reminders" | null
  >(null);
  ...
  const BASE_URL = `${apiUrl}/api/task`;

  useFocusEffect(
    useCallback(() => {
      loadReminders();
      fetchAssignedTaskHistory();
    }, []),
  );
  ...
  const toggleReminder = async (id: string) => {
    await saveReminders(
      reminders.map((r) => (r._id === id ? { ...r, done: !r.done } : r)),
    );
  };
...
const fetchAssignedTaskHistory = async () => {
  try {
    ...
    const res = await fetch(`${BASE_URL}/history/assigned`, {
      method: "GET",
      headers: {
    ...
    // Filter validation safe-check
    console.log("HISTORY TASK COUNT FOR NEW ADMIN:", data.length);
    setAssignedTasks(Array.isArray(data) ? data : []);
  } catch (error) {
    console.log("ASSIGNED TASK HISTORY ERROR:", error);
    setAssignedTasks([]);
  } finally {
  ...
  const handleAddTask = async () => {
    if (!newTaskTitle.trim()) {
      Alert.alert("Validation", "Task title required");
      return;
    }
  ...
  const isSameDay = (dateString?: string, targetDate?: Date) => {
    ...
    return startOfDay(taskDate).getTime() === startOfDay(targetDate).getTime();
  };

  const getAssignedUserNames = (assignedTo: TaskItem["assignedTo"]) => {
    if (!assignedTo) return "No user";
    ...
    return assignedTo.name || "No user";
  };

  const renderAssignedHistorySection = (title: string, tasks: TaskItem[]) => (
    <View style={aS.historySection}>
      <Text style={aS.historySectionTitle}>{title}</Text>

      {tasks.length === 0 ? (
        <Text style={aS.historyEmptyText}>No assigned tasks</Text>
      ) : (
        tasks.map((item) => (
          <View key={item._id} style={aS.historyTaskRow}>
            <View style={aS.historyDot} />

            <View style={{ flex: 1 }}>
              <Text style={aS.historyTaskTitle}>{item.title}</Text>

              <Text style={aS.historyTaskMeta}>
                Assigned to: {getAssignedUserNames(item.assignedTo)}
              </Text>
  ...
  const todayAssignedTasks = assignedTasks.filter((task) =>
    isSameDay(task.createdAt, new Date()),
  );

  const yesterdayAssignedTasks = assignedTasks.filter((task) =>
    isSameDay(task.createdAt, getDaysAgoDate(1)),
  );

  const twoDaysAgoAssignedTasks = assignedTasks.filter((task) =>
    isSameDay(task.createdAt, getDaysAgoDate(2)),
  );

  const last7DaysAssignedTasks = assignedTasks.filter((task) => {
    ...
    return taskDate >= sevenDaysAgo && taskDate < twoDaysAgo;
  });
  ...
  return (
    <SafeAreaView style={aS.safe}>
      ...
      >
        ...
        >
          <LinearGradient
            colors={["#5f00be", "#7b28db"]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
            style={aS.mainCardGrad}
          >
        ...
        {/* Assigned Task History Card */}
        <View style={aS.sectionCard}>
          <View style={aS.sectionCardHeader}>
            <View style={aS.cardLeft}>
              <View style={[aS.cardIconBox, { backgroundColor: "#F3E8FF" }]}>
                <Ionicons name="list-outline" size={22} color="#7b28db" />
              </View>

              <View>
                <Text style={aS.sectionCardTitle}>Assigned Task History</Text>
                <Text style={aS.sectionCardSub}>
                  Today {todayAssignedTasks.length} • Last 7 days{" "}
                  {
                    assignedTasks.filter((task) => {
                      if (!task.createdAt) return false;
                      const taskDate = startOfDay(new Date(task.createdAt));
                      return taskDate >= getDaysAgoDate(7);
                    }).length
                  }
                </Text>
              </View>
            ...
            <TouchableOpacity
              style={[
                ...
                activeSection === "assignedHistory" && {
                  backgroundColor: "#7b28db",
                },
              ]}
              onPress={async () => {
                if (activeSection === "assignedHistory") {
                  setActiveSection(null);
                  return;
                }

                setActiveSection("assignedHistory");
                await fetchAssignedTaskHistory();
              }}
              activeOpacity={0.8}
            >
              <Ionicons
                ...
                size={20}
                color={activeSection === "assignedHistory" ? "#fff" : "#7b28db"}
              />
            </TouchableOpacity>
          </View>
          ...
          {activeSection === "assignedHistory" && (
            <View style={aS.taskListBox}>
              {loadingAssignedTasks ? (
                ...
                <>
                  {renderAssignedHistorySection(
                    "Today Assigned Tasks",
                    todayAssignedTasks,
                  )}
                  {renderAssignedHistorySection(
                    "Yesterday Assigned Tasks",
                    yesterdayAssignedTasks,
                  )}
                  {renderAssignedHistorySection(
                    "2 Days Ago Assigned Tasks",
                    twoDaysAgoAssignedTasks,
                  )}
                  {renderAssignedHistorySection(
                    "Last 7 Days Assigned Tasks",
                    last7DaysAssignedTasks,
                  )}
                </>
              )}
            </View>
          ...
          <View style={aS.sectionCardHeader}>
            ...
            <View style={{ flexDirection: "row", gap: 8 }}>
              ...
              <TouchableOpacity
                style={[
                  aS.iconBtn,
                  activeSection === "myTask" && { backgroundColor: "#1D6FA4" },
                ]}
                onPress={async () => {
                  if (activeSection === "myTask") {
                    setActiveSection(null);
                    return;
                  }
              ...
              >
                <Ionicons
                  name={
                    activeSection === "myTask" ? "chevron-up" : "chevron-down"
                  }
                  size={20}
                  color={activeSection === "myTask" ? "#fff" : "#1D6FA4"}
                />
              </TouchableOpacity>
            </View>
          </View>
                  ...
                  >
                    <View
                      style={[
                        ...
                        {
                          backgroundColor:
                            item.status === "Completed" ? "#1D9E75" : "#5f00be",
                        },
                      ]}
                    />
        ...
        <View style={aS.sectionCard}>
          <View style={aS.sectionCardHeader}>
            ...
            <View style={{ flexDirection: "row", gap: 8 }}>
              <TouchableOpacity
                style={[aS.iconBtn, { backgroundColor: "#EDE7F6" }]}
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  setActiveSection(
                    activeSection === "reminders" ? null : "reminders",
                  )
                }
                activeOpacity={0.8}
              >
                <Ionicons
                  ...
                  size={20}
                  color={activeSection === "reminders" ? "#fff" : "#7B1FA2"}
                />
              </TouchableOpacity>
            </View>
          </View>
            ...
            <View style={aS.taskListBox}>
              {reminders.length === 0 ? (
                <View style={aS.emptyBox}>
                  ...
                  <TouchableOpacity
                    onPress={() => setAddReminderVisible(true)}
                    style={[aS.createFirstBtn, { backgroundColor: "#EDE7F6" }]}
                  >
                    <Text style={[aS.createFirstText, { color: "#7B1FA2" }]}>
                      + Add Reminder
                    </Text>
                  </TouchableOpacity>
                </View>
                ...
                reminders.map((r) => (
                  <View
                    key={r._id}
                    style={[aS.taskRow, r.done && { opacity: 0.5 }]}
                  >
                    <Switch
                      value={r.done}
                    ...
                    <View style={{ flex: 1, marginLeft: 10 }}>
                      ...
                      {r.note ? (
                        <Text style={aS.taskMeta}>{r.note}</Text>
                      ) : null}

                      {r.dueDate ? (
                        <Text style={[aS.taskMeta, { color: "#7B1FA2" }]}>
      ...
      >
        ...
        >
          ...
          <View style={aS.modalSheet}>
            ...
            <TextInput
              style={aS.textInput}
              placeholder="e.g. Review report, Fix bug..."
            ...
            >
              {savingTask ? (
                ...
                <Text style={aS.saveBtnText}>Add Task</Text>
              )}
            </TouchableOpacity>
          </View>
        </KeyboardAvoidingView>
      ...
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => setAddReminderVisible(false)}
      >
        <KeyboardAvoidingView
          style={{ flex: 1, justifyContent: "flex-end" }}
          behavior={Platform.OS === "ios" ? "padding" : "height"}
        >
          <Pressable
            style={aS.modalBg}
            onPress={() => setAddReminderVisible(false)}
          />
          ...
          <View style={aS.modalSheet}>
            ...
            <TextInput
              style={aS.textInput}
              placeholder="e.g. Meeting at 3pm..."
              placeholderTextColor="#9CA3AF"
              value={reminderTitle}
            ...
            <TextInput
              style={[aS.textInput, { height: 70, textAlignVertical: "top" }]}
              placeholder="Details..."
              placeholderTextColor="#9CA3AF"
              value={reminderNote}
            ...
            >
              {savingReminder ? (
                ...
                <Text style={aS.saveBtnText}>Save Reminder</Text>
              )}
            </TouchableOpacity>
          </View>
        </KeyboardAvoidingView>
...
function UserTaskScreen() {
  ...
  const filteredTasks = useMemo(() => {
    if (activeTab === "Pending") {
      return tasks.filter(
        (t) => t.status === "Pending" || t.status === "Reassigned",
      );
    }
  ...
  const pendingCount = useMemo(
    () =>
      tasks.filter((t) => t.status === "Pending" || t.status === "Reassigned")
        .length,
    [tasks],
  );
  ...
  const handleToggleTask = async (taskId: string, description: string) => {
    try {
      ...
      setTasks((prev) =>
        prev.map((item) => (item._id === taskId ? data : item)),
      );
    } catch {
        ...
        >
            ...
            <View style={uS.iconTitleWrap}>
              <View
                style={[
                  uS.taskIconWrap,
                  isCompleted
                    ? uS.completedIconWrap
                    : isReassigned
                      ? uS.reassignedIconWrap
                      : uS.pendingIconWrap,
                ]}
              >
                <Ionicons
              ...
              <View style={uS.taskTextWrap}>
                <Text
                  numberOfLines={2}
                  style={[uS.taskTitle, isCompleted && uS.completedTaskTitle]}
                >
                  {item.title}
                </Text>

                <Text style={uS.taskDate}>
                  {item.project?.name || "No project"} •{" "}
                  {formatTaskDate(item.createdAt)}
                </Text>
              </View>
            </View>
          ...
          {!isCompleted && (
            <TouchableOpacity
              ...
              style={uS.actionBtnWrap}
            >
              <LinearGradient
                colors={
                  isReassigned ? ["#378ADD", "#185FA5"] : ["#127a6e", "#081B43"]
                }
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 0 }}
                style={uS.taskActionButton}
              >
                <Ionicons
        ...
        ListHeaderComponent={
          <>
            ...
            <View style={uS.statsRow}>
              <LinearGradient
                colors={["#081B43", "#102B63"]}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
                style={uS.statCard}
              >
                <Text style={uS.statValue}>{tasks.length}</Text>
                <Text style={uS.statLabel}>Total</Text>
              </LinearGradient>
              ...
              <LinearGradient
                ...
                style={uS.statCard}
              >
                <Text style={uS.statValue}>{pendingCount}</Text>
                <Text style={uS.statLabel}>Pending</Text>
              </LinearGradient>

              <LinearGradient
                colors={["#127a6e", "#19A08F"]}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
                style={uS.statCard}
              >
                <Text style={uS.statValue}>{completedCount}</Text>
                <Text style={uS.statLabel}>Completed</Text>
              </LinearGradient>
            </View>

            <View style={uS.tabRow}>
              <TouchableOpacity
                style={[uS.tabBtn, activeTab === "Pending" && uS.tabBtnActive]}
                onPress={() => setActiveTab("Pending")}
              >
                <Text
                  style={[
                    uS.tabText,
                    activeTab === "Pending" && uS.tabTextActive,
                  ]}
                >
              ...
              <TouchableOpacity
                style={[
                  uS.tabBtn,
                  activeTab === "Completed" && uS.tabBtnActiveGreen,
                ]}
              ...
              >
                <Text
                  style={[
                    uS.tabText,
                    activeTab === "Completed" && uS.tabTextActive,
                  ]}
                >
      ...
      >
        ...
        >
          ...
          <View style={uS.modalSheet}>
            ...
            <Text style={uS.modalSub}>
              Kya kiya — thoda describe karo (optional)
            </Text>

            <TextInput
              style={uS.modalInput}
              placeholder="e.g. Kaam complete ho gaya..."
            ...
            <View style={uS.modalBtns}>
              ...
              >
                <LinearGradient
                  colors={["#127a6e", "#081B43"]}
                  style={uS.modalBtnGrad}
                >
